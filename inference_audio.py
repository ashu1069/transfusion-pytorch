...
def main():
    ...
    frames = int(args.duration * SAMPLE_RATE / HOP_LENGTH) + 1 if args.duration else None
    
    with torch.no_grad():
        kwargs = {'batch_size': args.num, 'modality_steps': args.steps}
        if frames: kwargs['fixed_modality_shape'] = (frames,)
    ...
    for i, mel in enumerate(mels):
        try:
            ...
            torchaudio.save(str(out / f'sample_{i}.wav'), audio.cpu().unsqueeze(0), SAMPLE_RATE)
